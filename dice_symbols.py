...
def dice_symbol_table():
  ...
  #symbol_table.add_infix("v", 10) "not implemented yet"
  #symbol_table.add_infix("^", 10) "not implemented yet"
  ...
  #symbol_table.add_infix("#", 55) "not implemented yet"
  ...
  def dz_led(self, left, parser):
    ...
    self.first = left
    operators = set()
    if parser.token.id != "(quote)" or parser.token.id != "(":
      while True:
        if parser.token.id in allowed_operators:
          ...
          break
    self.third = ' '.join([f"(quote {op})" for op in operators])
